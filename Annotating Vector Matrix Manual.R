...
A<-newd%>%mutate(Anemia=cut(Hbg,breaks = c(0,11.9,100),labels = c("<12",">12")),
  ...
  geom_point()+stat_cor(method = "kendall",cor.coef.name="tau",
                        label.x.npc ="middle",label.y.npc = "center")+
...
D<-table%>%mutate(
  ...
  ggplot(.,aes(x=CD4,y=PLAT,col=grp))+geom_point()+stat_cor(
...
test_A<-newd%>%mutate(Anemia=cut(Hbg,breaks = c(0,11.9,100),labels = c("<12",">12")),
                 Leucopenia=cut(Wbc,breaks = c(0,3.9,100),labels = c("<4",">4")),
                 Lymphocytopenia=cut(Lymp,breaks = c(0,0.7,100),labels = c("<0.8",">0.8")),
                 Throbocytopenia=cut(Plat,breaks = c(0,149,Inf),labels = c("<150",">150")),
                 immune=cut(Cd4,breaks = c(0,199,Inf),labels = c("<200",">200")))%>%
...
annotation<-newd%>%mutate(
  Anemia=cut(Hbg,breaks = c(0,11.9,100),labels = c("<12",">12")),
  Leucopenia=cut(Wbc,breaks = c(0,3.9,100),labels = c("<4",">4")),
  Lymphocytopenia=cut(Lymp,breaks = c(0,0.7,100),labels = c("<0.8",">0.8")),
  Throbocytopenia=cut(Plat,breaks = c(0,149,Inf),labels = c("<150",">150")),
  immune=cut(Cd4,breaks = c(0,199,Inf),labels = c("<200",">200")))%>%
  mutate(xend= case_when(
   Anemia=="<12"&immune=="<200"~150,
   Anemia==">12"&immune=="<200"~150,
   Anemia=="<12"&immune==">200"~800,
   Anemia==">12"&immune==">200"~800
   ),
   yend= case_when(
   Anemia=="<12"&immune=="<200"~5,
   Anemia==">12"&immune=="<200"~15.5,
   Anemia=="<12"&immune==">200"~5,
   Anemia==">12"&immune==">200"~15.5),
   prettys= case_when(
    Anemia=="<12"&immune=="<200"~"tau=0.044,p=0.70 \n tau=0.19,0=0.32",
    Anemia=="<12"&immune==">200"~"tau=0.036,p=0.9\n tau=0.18,p=0.38",
    Anemia==">12"&immune=="<200"~"tau=0.41,p=0.25\n tau=0.2,p=0.54",
    Anemia==">12"&immune==">200"~"tau=0.029,p=0.92\n tau=-0.79,p=0.0.0055")
    )%>%
  mutate(Anemia=factor(
    Anemia,labels = c("<12"=expression(bold("Anemic")),
                      ">12"=expression(bold("non-Anemic")))),
    immune=factor(immune, labels = c("<200"=expression(bold("<200")),
                      ">200"=expression(bold("">="200")))))
test_A+ geom_text(data = annotation,aes(label=prettys,x=xend , y=yend),col="blue")
